...
control 'V-213930' do
  title "SQL Server must integrate with an organization-level
...
account management requirements.
  ...
  desc  'rationale', ''
  desc  'check', "
    Determine whether SQL Server is configured to use only Windows
authentication.

    In the Object Explorer in SQL Server Management Studio (SSMS), right-click
on the server instance.
    ...
    In a query interface such as the SSMS Transact-SQL editor, run the
statement:
...
Authentication\", this is not a finding.
    ...
    Mixed mode (both SQL Server authentication and Windows authentication) is
in use. If the need for mixed mode has not been documented and approved, this
is a finding.

    From the documentation, obtain the list of accounts authorized to be
managed by SQL Server.

    Determine the accounts (SQL Logins) actually managed by SQL Server. Run the
statement:
    ...
    If any accounts listed by the query are not listed in the documentation,
this is a finding.
  "
  desc  'fix', "
    If mixed mode is required, document the need and justification; describe
the measures taken to ensure the use of SQL Server authentication is kept to a
minimum; describe the measures taken to safeguard passwords; list or describe
the SQL Logins used.
...
N'Software\\Microsoft\\MSSQLServer\\MSSQLServer', N'LoginMode', REG_DWORD, 2
    ...
    For each account being managed by SQL Server but not requiring it, drop or
disable the SQL Login. Replace it with an appropriately configured account, as
needed.
    ...
    Dropping a Login does not delete the equivalent database User(s). There may
be more than one database containing a User mapped to the Login. Drop the
User(s) unless still needed.
