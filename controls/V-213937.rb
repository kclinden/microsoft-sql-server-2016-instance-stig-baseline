...
control 'V-213937' do
  title "SQL Server must allow only the ISSM (or individuals or roles appointed
...
identify those responsible for one.
  ...
  desc  'rationale', ''
  desc  'check', "
    Obtain the list of approved audit maintainers from the system
documentation.

    Review the server roles and individual logins that have the following role
memberships, all of which enable the ability to create and maintain audit
definitions.
...
definitions.
    ...
    Use the following query to determine the roles and logins that have the
listed permissions:
...
'SERVER (linked)'
    ...
    WHERE sp.permission_name IN ('ALTER ANY SERVER AUDIT','CONTROL
SERVER','ALTER ANY DATABASE','CREATE ANY DATABASE')
    OR R.name IN ('sysadmin','dbcreator')

    If any of the logins, roles, or role memberships returned have permissions
that are not documented, or the documented audit maintainers do not have
permissions, this is a finding.
  "
  desc  'fix', "
    Create a server role specifically for audit maintainers and give it
permission to maintain audits without granting it unnecessary permissions (the
role name used here is an example; other names may be used):
...
statements to remove the ALTER ANY SERVER AUDIT permission from all logins.
