...
users.
    ...
    A privileged function in SQL Server/database context is any operation that
...
table executed by other than a security principal.
    ...
    Depending on the capabilities of SQL Server and the design of the database
...
triggers, other mechanisms, or a combination of these.
  ...
  desc  'rationale', ''
  desc  'check', "
    Review server-level securables and built-in role membership to ensure only
authorized users have privileged access and the ability to create server-level
objects and grant permissions to themselves or others.
...
protection for DBMS server securables, by type of login.
    ...
    Review the permissions in place on the server. If the actual permissions do
not match the documented requirements, this is a finding.
...
X.member_principal_id
    ...
    The CONTROL SERVER permission is similar but not identical to the sysadmin
fixed server role. Permissions do not imply role memberships and role
memberships do not grant permissions. (e.g., CONTROL SERVER does not imply
membership in the sysadmin fixed server role.)

    Ensure only the documented and approved logins have privileged functions in
SQL Server.

    If the current configuration does not match the documented baseline, this
is a finding.
  "
  desc  'fix', "Restrict the granting of permissions to server-level securables
to only those authorized. Most notably, members of sysadmin and securityadmin
built-in instance-level roles, CONTROL SERVER permission, and use of the GRANT
with GRANT permission."
  impact 0.5
  tag severity: 'medium'
  tag gtitle: 'SRG-APP-000340-DB-000304'
  tag gid: 'V-213979'
  tag rid: 'SV-213979r508026_rule'
  tag stig_id: 'SQL6-D0-010400'
  tag fix_id: 'F-15194r313721_fix'
  tag cci: ['SV-93925', 'V-79219', 'CCI-002235']
  tag nist: ['AC-6 (10)']
end
