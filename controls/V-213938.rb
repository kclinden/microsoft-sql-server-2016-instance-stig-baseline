...
determine if any and every action on the database is permitted.
  ...
  desc  'rationale', ''
  desc  'check', "
    Review the system documentation to determine if SQL Server is required to
audit the retrieval of privilege/permission/role membership information.

    If SQL Server is not required to audit the retrieval of
privilege/permission/role membership information, this is not a finding.
...
query. If no records are returned, this is a finding.
    ...
    If the auditing the retrieval of privilege/permission/role membership
information is required, execute the following query to verify the
SCHEMA_OBJECT_ACCESS_GROUP is included in the server audit specification:
...
'SCHEMA_OBJECT_ACCESS_GROUP'
    ...
    If the SCHEMA_OBJECT_ACCESS_GROUP is not returned in an active audit, this
is a finding.
  "
  desc  'fix', "
    If SQL Server is required to audit the retrieval of
privilege/permission/role membership information, create a dedicated audit to
capture this information.
    ...
    DECLARE @auditName varchar(50), @auditPath varchar(260), @auditGuid
uniqueidentifier, @auditFileSize varchar(4), @auditFileCount varchar(4)
    ...
    SET @auditPath = 'C:\\Program Files\\Microsoft SQL
Server\\MSSQL13.SQL2016\\MSSQL\\Audits'
    ...
    Define the number of files that should be kept online. Use -1 for
unlimited:
    SET @auditFileCount = 50

    Insert the variables into a temp table so they survive for the duration of
the script:
    ...
    SET  @auditName = (SELECT Value FROM #SetupVars WHERE Variable =
'auditName')
    SET  @disableSpecification = '
    IF EXISTS (SELECT 1 FROM sys.server_audit_specifications WHERE name = N'''
+ @auditName + '_SERVER_SPECIFICATION'')
...
''')
    ...
    DECLARE @auditName varchar(50), @auditPath varchar(260), @auditGuid
varchar(40), @auditFileSize varchar(4), @auditFileCount varchar(5)
    ...
    SELECT @auditFileSize = Value FROM #SetupVars WHERE Variable =
'auditFileSize'
    SELECT @auditFileCount = Value FROM #SetupVars WHERE Variable =
'auditFileCount'
    ...
    (
     ...
     , MAX_ROLLOVER_FILES = ' + CASE WHEN @auditFileCount = -1 THEN 'UNLIMITED'
ELSE @auditFileCount END + '
    ...
    OR ([Schema_Name] = ''sys'' AND [Object_Name] =
...
'auditName')
  ...
  tag gid: 'V-213938'
  tag rid: 'SV-213938r508026_rule'
  tag stig_id: 'SQL6-D0-004500'
  tag fix_id: 'F-15153r313598_fix'
  tag cci: ['SV-93843', 'V-79137', 'CCI-000172']
  tag nist: ['AU-12 c']
end
