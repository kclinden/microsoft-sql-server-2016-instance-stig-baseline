...
control 'V-213941' do
  title "SQL Server must include additional, more detailed,
...
identities of shared account users.
  ...
  desc  'rationale', ''
  desc  'check', "
    If a SQL Server Audit is not in use for audit purposes, this is a finding
...
audit any events, and any fields, in addition to those in the standard audit.

    If there are none specified, this is not a finding.

    If SQL Server Audit is in use, compare the audit specification(s) with the
...
by supplemental, locally-deployed mechanisms), this is a finding.
  "
  desc  'fix', "
    Design and deploy an Audit that captures all auditable events and data
items. In the event a third-party tool is used for auditing it must contain all
the required information including but not limited to events, type, location,
subject, date and time and by whom the change occurred.

    Implement additional custom audits to capture the additional organizational
required information.
  ...
  tag gid: 'V-213941'
  tag rid: 'SV-213941r508026_rule'
  tag stig_id: 'SQL6-D0-005500'
  tag fix_id: 'F-15156r313607_fix'
  tag cci: ['V-79145', 'SV-93851', 'CCI-000135']
  tag nist: ['AU-3 (1)']
end
