...
during initial installation of SQL Server and is closely associated with the
...
central log management system; and any limitations that exist on SQL Server's
...
Locate the Audit file path and drive.
    ...
    Calculate the space needed as the maximum file size and number of files
from the SQL Audit File properties.

    If the calculated product of the \"max_file_size\" times the
\"max_rollover_files\" exceeds the size of the storage location or if
\"max_file_size\" or \"max_rollover_files\" are set to \"0\" (UNLIMITED), this
is a finding.
  "
  desc  'fix', "
    Review the SQL Audit file location, ensure the destination has enough space
available to accommodate the maximum total size of all files that could be
written.

    Configure the maximum number of audit log files that are to be generated,
staying within the number of logs the system was sized to support.

    Update the \"max_files\" parameter of the audits to ensure the correct
number of files is defined.
  "
  impact 0.5
  tag severity: 'medium'
  tag gtitle: 'SRG-APP-000357-DB-000316'
  tag gid: 'V-213983'
  tag rid: 'SV-213983r508026_rule'
  tag stig_id: 'SQL6-D0-010900'
  tag fix_id: 'F-15198r313733_fix'
  tag cci: ['V-79227', 'SV-93933', 'CCI-001849']
  tag nist: ['AU-4']
end
