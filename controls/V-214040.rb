...
control 'V-214040' do
  title "Remote Data Archive feature must be disabled, unless specifically
...
and enabling them could adversely affect the security of the system.
    ...
    The Remote Data Archive feature allows an export of local SQL Server data
to an Azure SQL Database. An exploit to the SQL Server instance could result in
a compromise of the host system and external SQL Server resources.
  "
  desc  'rationale', ''
  desc  'check', "
    To determine if \"Remote Data Archive\" option is enabled, execute the
following query:
...
to determine whether the use of \"Remote Data Archive\" is required and
authorized. If it is not authorized, this is a finding.
  "
  desc  'fix', "
    Disable use of or remove any external application executable object
definitions that are not authorized. To disable the use of \"Remote Data
Archive\" option, from the query prompt: 
  ...
  tag gid: 'V-214040'
  tag rid: 'SV-214040r508026_rule'
  tag stig_id: 'SQL6-D0-017600'
  tag fix_id: 'F-15255r313904_fix'
  tag cci: ['SV-94051', 'V-79345', 'CCI-000381']
  tag nist: ['CM-7 a']
end
