...
control 'V-213931' do
  title "SQL Server must be configured to utilize the most-secure
...
account usage.
    ...
    SQL Server must be configured to automatically utilize organization-level
...
account management requirements.
    ...
    SQL Server supports several authentication methods to allow operation in
various environments, Kerberos, NTLM, and SQL Server. An instance of SQL Server
must be configured to utilize the most-secure method available. Service
accounts utilized by SQL Server should be unique to a given instance.
  "
  desc  'rationale', ''
  desc  'check', "
    If the SQL Server is not part of an Active Directory domain, this finding
is Not Applicable.
    ...
    Right-click on \"Computer\" or \"This PC\" (Varies by OS level), click
\"Properties\".

    Note the value shown for \"Full computer name\".

    *** Note: For a cluster, this value must be obtained from the Failover
Cluster Manager. ***

    Obtain the TCP port that is supporting the SQL Server instance:

    Click Start >> Type \"SQL Server 2016 Configuration Manager\" >> From the
search results, click \"SQL Server 2016 Configuration Manager\".

    From the tree on the left, expand \"SQL Server Network Configuration\".

    Click \"Protocols for <Instance Name>\" where <Instance Name> is the name
of the instance (MSSQLSERVER is the default name).
    ...
    Note the account listed in the \"Log On As\" column for the SQL Server
instance being reviewed.

    Launch a command-line or PowerShell window.

    Enter the following command where <Service Account> is the identity of the
service account.
    ...
    If the listing does not contain the following supported service principal
names (SPN) formats, this is a finding.
    ...
    If the MSSQLSvc service is registered for any fully qualified domain names
that do not match the current server, this may indicate the service account is
shared across SQL Server instances. Review server documentation, if the sharing
of service accounts across instances is not documented and authorized, this is
a finding.
  "
  desc  'fix', "
    Ensure Service Principal Names (SPNs) are properly registered for the SQL
Server instance.

    Utilize the Microsoft Kerberos Configuration Manager to review Kerberos
configuration issues for a given SQL Server instance.

    https://www.microsoft.com/en-us/download/details.aspx?id=39046

    Alternatively, SPNs for SQL Server can be manually registered.

    For other connections that support Kerberos the SPN is registered in the
format MSSQLSvc/<FQDN>/<instancename> for a named instance. The format for
registering the default instance is MSSQLSvc/<FQDN>.

    Using an account with permissions to register SPNs, issue the following
commands from a command-prompt:

    setspn -S MSSQLSvc/<Fully Qualified Domain Name> <Service Account>
    setspn -S MSSQLSvc/<Fully Qualified Domain Name>:<TCP Port> <Service
Account>
...
https://docs.microsoft.com/en-us/sql/database-engine/configure-windows/register-a-service-principal-name-for-kerberos-connections#Manual
  ...
  tag gid: 'V-213931'
  tag rid: 'SV-213931r508026_rule'
  tag stig_id: 'SQL6-D0-003800'
  tag fix_id: 'F-15146r457859_fix'
  tag cci: ['V-79123', 'SV-93829', 'CCI-000015']
  tag nist: ['AC-2 (1)']
end
