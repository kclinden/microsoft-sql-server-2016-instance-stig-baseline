...
control 'V-213943' do
  title "SQL Server must be configurable to overwrite audit log records, oldest
...
SQL Server must continue generating audit records, if possible (automatically
...
communication with this server is lost or the server fails, SQL Server must
...
situations.
  ...
  desc  'rationale', ''
  desc  'check', "
    If the system documentation indicates that availability does not take
precedence over audit trail completeness, this is not applicable (NA).
    ...
    If the \"storage_type\" is \"APPLICATION LOG\" or \"SECURITY LOG\", this is
not a finding.

    If the \"storage_type\" is \"FILE\" and \"max_rollover_files\" is greater
than zero, this is not a finding. Otherwise, this is a finding.

  "
  desc  'fix', "
    If SQL Server Audit is in use, configure SQL Server Audit to continue to
...
an auditing failure.
  ...
  tag gid: 'V-213943'
  tag rid: 'SV-213943r508026_rule'
  tag stig_id: 'SQL6-D0-005700'
  tag fix_id: 'F-15158r313613_fix'
  tag cci: ['V-79149', 'SV-93855', 'CCI-000140']
  tag nist: ['AU-5 b']
end
