...
control 'V-214039' do
  title "Allow Polybase Export feature must be disabled, unless specifically
...
and enabling them could adversely affect the security of the system.
    ...
    The Allow Polybase Export feature allows an export of data to an external
data source such as Hadoop File System or Azure Data Lake. An exploit to the
SQL Server instance could result in a compromise of the host system and
external SQL Server resources.
  "
  desc  'rationale', ''
  desc  'check', "
    To determine if \"Allow Polybase Export\" option is enabled, execute the
following query:
...
to determine whether the use of \"Allow Polybase Export\" is required and
authorized. If it is not authorized, this is a finding.
  "
  desc  'fix', "
    Disable use of or remove any external application executable object
definitions that are not authorized. To disable the use of \"Allow Polybase
Export\" option, from the query prompt: 
  ...
  tag gid: 'V-214039'
  tag rid: 'SV-214039r508026_rule'
  tag stig_id: 'SQL6-D0-017500'
  tag fix_id: 'F-15254r313901_fix'
  tag cci: ['SV-94049', 'V-79343', 'CCI-000381']
  tag nist: ['CM-7 a']
end
