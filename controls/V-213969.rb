...
modules are described in FIPS 140-2 Annex A.
    ...
    NSA Type- (where =1, 2, 3, 4) products are NSA-certified, hardware-based
encryption modules.
  "
  desc  'rationale', ''
  desc  'check', "
    In Windows, open Administrative Tools >> Local Security Policy. Expand
Local Policies >> Security Options. In the right-side pane, find \"System
cryptography: Use FIPS compliant algorithms for encryption, hashing, and
signing.\"

    If, in the \"Security Setting\" column, the value is \"Disabled,\" this is
a finding.
  ...
  desc  'fix', "
    In Windows, open Administrative Tools >> Local Security Policy. Expand
Local Policies >> Security Options. In the right-side pane, double-click on
\"System cryptography: Use FIPS compliant algorithms for encryption, hashing,
and signing.\"

    In the dialog box that appears, if the radio buttons are active, click
\"Enabled\", and then click \"Apply\". If the radio buttons are grayed out, use
Group Policy Management (on the appropriate server for this domain) to enforce
the Enabled policy, and deploy it to the server(s) running SQL Server.
  "
  impact 0.5
  tag severity: 'medium'
  tag gtitle: 'SRG-APP-000179-DB-000114'
  tag gid: 'V-213969'
  tag rid: 'SV-213969r508026_rule'
  tag stig_id: 'SQL6-D0-008700'
  tag fix_id: 'F-15184r313691_fix'
  tag cci: ['SV-93905', 'V-79199', 'CCI-000803']
  tag nist: ['IA-7']
end
