...
control 'V-213942' do
  title "SQL Server must by default shut down upon audit failure, to include
the unavailability of space for more audit log records; or must be configurable
to shut down upon audit failure."
  desc  "It is critical that when SQL Server is at risk of failing to process
audit logs as required, it takes action to mitigate the failure. Audit
...
the most extreme situations.
  ...
  desc  'rationale', ''
  desc  'check', "
    If the system documentation indicates that availability takes precedence
over audit trail completeness, this is not applicable (NA).

    If SQL Server Audit is in use, review the defined server audits by running
the statement:
...
rows in use.
    ...
    If the [on_failure_desc] is \"SHUTDOWN SERVER INSTANCE\" on this/these
row(s), this is not a finding. Otherwise, this is a finding.
  "
  desc  'fix', "
    If SQL Server Audit is in use, configure SQL Server Audit to shut SQL
Server down upon audit failure, to include running out of space for audit logs.
  ...
  tag gid: 'V-213942'
  tag rid: 'SV-213942r508026_rule'
  tag stig_id: 'SQL6-D0-005600'
  tag fix_id: 'F-15157r313610_fix'
  tag cci: ['V-79147', 'SV-93853', 'CCI-000140']
  tag nist: ['AU-5 b']
end
