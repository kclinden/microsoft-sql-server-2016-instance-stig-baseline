...
control 'V-213972' do
  title "SQL Server must protect the confidentiality and integrity of all
information at rest."
...
the data will be open to compromise and unauthorized modification.
  ...
  desc  'rationale', ''
  desc  'check', "
    Review system documentation to determine whether the system handles
classified information. If the system does not handle classified information,
the severity of this check should be downgraded to Category II.
...
encrypted.
    ...
    If data encryption is required, ensure the data is encrypted before being
put on the secondary device by executing:
    ...
    For each user database where encryption is required, verify that encryption
...
data on a secondary device. Where encryption is required, this can be done by
full-disk encryption or by database encryption.

    To enable database encryption, create a master key, create a database
encryption key, and protect it by using mechanisms tied to the master key, and
then set encryption on.

    Implement physical security measures, operating system access control lists
and organizational controls appropriate to the sensitivity level of the data in
the database(s).
  "
  impact 0.7
  tag severity: 'high'
  ...
  tag gid: 'V-213972'
  tag rid: 'SV-213972r508026_rule'
  tag stig_id: 'SQL6-D0-009500'
  tag fix_id: 'F-15187r313700_fix'
  tag cci: ['V-79205', 'SV-93911', 'CCI-001199']
  tag nist: ['SC-28']
end
