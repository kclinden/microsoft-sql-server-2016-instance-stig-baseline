...
consequences, including the complete loss of control over SQL Server. If the
[sa] default account is not disabled, an attacker might be able to gain access
through the account. SQL Server by default, at installation, disables the [sa]
account.
...
that require the [sa] account to be enabled are usually legacy systems.
  ...
  desc  'rationale', ''
  desc  'check', "
    Check SQL Server settings to determine if the [sa] (system administrator)
account has been disabled by executing the following query:
    ...
    Verify that the \"name\" column contains the current name of the [sa]
database server account.

    If the \"is_disabled\" column is not set to “1”, this is a finding.
  "
  desc  'fix', "
    Modify the enabled flag of SQL Server's [sa] (system administrator) account
by running the following script.
  ...
  tag gid: 'V-214028'
  tag rid: 'SV-214028r508026_rule'
  tag stig_id: 'SQL6-D0-016200'
  tag fix_id: 'F-15243r313868_fix'
  tag cci: ['V-79317', 'SV-94023', 'CCI-000213']
  tag nist: ['AC-3']
end
